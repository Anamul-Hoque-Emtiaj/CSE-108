...
class ParallelSquareCounter implements Runnable {
    ...
    public void run() {
        for(int i=startIndex;i<startIndex+NUMBER_COUNT;i++){
            ...
            int Sqrt = (int) sqrt(currentNumber);
            if(Sqrt*Sqrt==currentNumber)
                squareCount++;
        }
        System.out.println(Thread.currentThread().getName()+": "+squareCount);
    }
}
...
public class Main {
    public static void main(String[] args) {
        ...
        for(int i=0;i<THREAD_COUNT;i++){
            int startIndex = i*NUMBER_COUNT/THREAD_COUNT;
            parallelSquareCounters[i] = new ParallelSquareCounter(numbers,startIndex);
            threads[i] = new Thread(parallelSquareCounters[i]);
            threads[i].start();
        }
        int totalSquareNumber=0;
        for(int i=0;i<THREAD_COUNT;i++){
            try {
                threads[i].join();
                totalSquareNumber += parallelSquareCounters[i].getSquareCount();
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
        System.out.println("Total number of Square: "+totalSquareNumber);
    }
}